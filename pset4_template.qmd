...
4. "This submission is our work alone and complies with the 30538 integrity policy." Add your initials to indicate your agreement: xinyi\*\*\_\_\*\* \*\*\_\_\*\*
5. "I have uploaded the names of anyone else other than my partner and I worked with on the problem set **[here](https://docs.google.com/forms/d/185usrCREQaUbvAXpWhChkjghdGgmAZXA3lPWpXLLsts/edit)**" xyz  (1 point)
6. Late coins used this pset: xyz\*\*\_\_\*\* Late coins left after submission: xyz\*\*\_\_\*\*
...
```{python}
pathshp = '~/Desktop/gz_2010_us_860_00_500k/gz_2010_us_860_00_500k.shp'
df_texas = gpd.read_file(pathshp)
df_texas = df_texas[df_texas["ZCTA5"].str.startswith(('75', '76', '77', '78', '79'))]
df_texas['ZCTA5'] = df_texas['ZCTA5'].astype(str)

map_texas = df_texas.merge(closures_zip, left_on='ZCTA5', right_on='ZIP_CD', how='left')
map_texas['closure_count'] = map_texas['closure_count'].fillna(0)

fig, ax = plt.subplots(1, 1, figsize=(12, 10))
map_texas.plot(column='closure_count', cmap='OrRd', linewidth=0.8, ax=ax, edgecolor='0.8', legend=True)
ax.set_title('Texas ZIP Codes Directly Affected by Hospital Closures (2016-2019)', fontsize=15)
plt.axis('off')
plt.show()

directly_affected_zip_count = closures_zip.shape[0]
print(f"Number of directly affected ZIP codes in Texas: {directly_affected_zip_count}")
```

3. 
```{python}
directly_affected_gdf = df_texas[df_texas['ZCTA5'].isin(closures_zip['ZIP_CD'])].copy()

directly_affected_gdf = directly_affected_gdf.to_crs(epsg=3395)
directly_affected_gdf['geometry'] = directly_affected_gdf.geometry.buffer(16093.4)
directly_affected_gdf = directly_affected_gdf.to_crs(epsg=4326) 

indirectly_affected_gdf = gpd.sjoin(df_texas, directly_affected_gdf, how='inner', predicate='intersects')
...
```{python}
indirectly_affected_gdf = indirectly_affected_gdf[~indirectly_affected_gdf['ZCTA5_left'].isin(closures_zip['ZIP_CD'])]

df_texas['category'] = 'Not Affected'
df_texas.loc[df_texas['ZCTA5'].isin(closures_zip['ZIP_CD']), 'category'] = 'Directly Affected'
df_texas.loc[df_texas['ZCTA5'].isin(indirectly_affected_gdf['ZCTA5_left']), 'category'] = 'Indirectly Affected'

fig, ax = plt.subplots(1, 1, figsize=(12, 10))
df_texas.plot(column='category', cmap='Set1', linewidth=0.8, ax=ax, edgecolor='0.8', legend=True)
ax.set_title('Texas ZIP Codes Affected by Hospital Closures (2016-2019)', fontsize=15)
plt.axis('off')
plt.show()
